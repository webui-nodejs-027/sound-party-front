...
const headRows = [
  ...
  { id: 'name', sort: true, disablePadding: true, label: 'Name' },
  { id: 'singer', sort: true, disablePadding: false, label: 'Singer' },
  { id: 'genre', sort: true, disablePadding: false, label: 'Genre' },
  { id: 'year', sort: true, disablePadding: false, label: 'Year' },
...
export default function SongTable() {
    ...
    if (sortBy) {
      const getData = async (sortBy, isDesc) => {
        const response = await fetch(
          `http://localhost:3000/api/songs?page=1&limit=${rowsPerPage}&sortBy=${sortBy}&order=${isDesc}`
        );
        const result = await response.json();
        const emptyData = [];
        setData(emptyData);
  ...
  const handleChangePage = (event, newPage) => {
    const getData = async () => {
      const response = await fetch(
        `http://localhost:3000/api/songs?page=${newPage +
          1}&limit=${rowsPerPage}`
      );
  ...
  const handleChangeRowsPerPage = event => {
    const getData = async () => {
      const response = await fetch(
        `http://localhost:3000/api/songs?page=1&limit=${+event.target.value}`
      );
      let result = await response.json();
      const emptyData = [];
      setData(emptyData);
                ...
                return (
                  <TableRow hover key={row.id}>
                    ...
                    <TableCell align="left">
                      <audio
                        src={`http://localhost:3000/music/${row.source}`}
