...
function SongTable(props) {
  ...
  const getSongs = async (pageParam, limitParam, sortByParam, orderByParam) => {
    let response = null;
    if (props.searchBy || props.location.state) {
      response = await fetch(
        `http://localhost:3001/api/songs?${props.searchBy ||
          props.location.state.searchBy}=${props.value ||
          props.location.state.value}&page=${pageParam}&limit=${limitParam}&sortBy=${sortByParam}&order=${orderByParam}`
      );
    } else {
      response = await fetch(
              ...
              {songs.map((song, index) => {
                return (
                  <TableRow hover key={song.id}>
                    <TableCell>
                      <PlayArrow
                        onClick={() => handleChangeMusic(song, index)}
                      />
                    </TableCell>
